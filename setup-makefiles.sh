...
LINEEND=" \\"
COUNT=`wc -l ../../IUNI/U810/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../../IUNI/U810/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../../IUNI/U810/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
...
LINEEND=" \\"
COUNT=`wc -l ../../IUNI/U810/proprietary-files-qc.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../../IUNI/U810/proprietary-files-qc.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../../IUNI/U810/proprietary-files-qc.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
